...
```
  ...
  nincs-séma-útvonal    = szegmens-nn-nkp *( "/" szegmens )
...
Az URI részerőforrás azonosító komponense lehetővé teszi a kiegészítő azonosítási
információt hordozó másodlagos erőforrások közvetett azonosítását, amelyek az
elsődleges erőforrásra hivatkoznak. Az így azonosított másodlagos erőforrás jelentheti
az elsődleges erőforrás valamely részét vagy részhalmazát. A részerőforrás azonosítót
(nevezik "nevesített horgonynak" is) a böngészők nem szokták elküldeni a szervernek
az URI többi részével, mivel általában az elsődleges erőforráson belül hivatkozik
egy horgonnyal (`anchor`) kijelölt megjelenítési pontra
(a html-kódban `<a name="horgony">Megjelenítési pont.</a>`), ami a böngésző számára
hordoz csak információt, konkrétan azt, hogy ettől a ponttól kell kezdeni a
tartalom megjelenítését. A részerőforrás azonosító komponens kezdetét a kettős
