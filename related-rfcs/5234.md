...
|   Szabálynév  |   Lokalizált név  |       Kifejezés       |       Megjegyzések       |
|---------------|-------------------|:---------------------:|:------------------------:|
| `rulelist` | `szabálylista` | `1*( szabály / (*megj-vagy-térköz megj-vagy-újsor) )` | folytatódik, ha a következő sor térközzel kezdődik |
| `rule` | `szabály` | `szabálynév legyen-egyenlő elemek megj-vagy-újsor` | folytatódik, ha a következő sor térközzel kezdődik |
| `rulename` | `szabálynév` | `ANGOLABC *(ANGOLABC / DECIMÁLIS / "-")` | |
| `defined-as` | `legyen-egyenlő` | `*megj-vagy-térköz ("=" / "=/") *megj-vagy-térköz` | alap vagy növekményes meghatározás jele |
| `elements` | `elemek` | `vagylagos-felsorolás *megj-vagy-térköz` | |
| `c-wsp` | `megj-vagy-térköz` | `TÉRKÖZ / (megj-vagy-újsor TÉRKÖZ)` | |
| `c-nl` | `megj-vagy-újsor` | `megjegyzés / SORVÉG` | megjegyzés vagy sorvége |
| `comment` | `megjegyzés` | `";" *(TÉRKÖZ / NYOMTATHATÓ) SORVÉG` | egysoros megjegyzés |
| `alternation` | `vagylagos-felsorolás` | `felsorolás *(*megj-vagy-térköz "/" *megj-vagy-térköz felsorolás)` | lehetséges opciók, perjellel elválasztva |
...
| `char-val` | `karakteres-érték` | `MACSKAKÖRÖM *(%x20-21 / %x23-7E) MACSKAKÖRÖM` | idézőjelekkel határolt, nyomtatható karaktereket és szóközt tartalmazó szöveg |
| `num-val` | `számérték` | `"%" (bináris-érték / decimális-érték / hexadecimális-érték)` | |
| `bin-val` | `bináris-érték` | `"b" 1*BIT [ 1*("." 1*BIT) / ("-" 1*BIT) ]` | bináris értékek sorozata vagy egyetlen érték |
...
| `prose-val` | `karakterlánc-érték` | `"<" *(%x20-3D / %x3F-7E) ">"` | csúcsos zárójelekkel határolt, nyomtatható karaktereket és szóközt tartalmazó karakterlánc, végső megoldásként használható |
